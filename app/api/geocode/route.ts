...
export async function GET(req: NextRequest) {
  ...
  const url = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
    q
  )}.json?types=place,locality,region&limit=5&access_token=${process.env.MAPBOX_SECRET_TOKEN}`

  const res = await fetch(url)
  const data = await res.json()

  const results =
    data.features?.map((f: { place_name: string; text: string; center: [number, number]; context?: Array<{ id: string; text: string; short_code?: string }> }) => {
      const countryContext = f.context?.find((c: { id: string }) =>
